...
- UI/input ownership status: page renderers, shared UI primitives, topbar/list/help chrome, and UI projection structs now live under `src/ui`; page-level input routing lives in `src/app/app_input_router.*`; `src/app` composes UI renderers, delegates input routing, and translates app/runtime/playback state into UI projections at the render boundary.
