...
- Architecture boundary governance: `scripts/check-architecture-boundaries.ps1` is wired into CI to prevent UI/page code from including concrete platform or backend layers, prevent `core` from depending upward, and prevent host/device adapters from depending on concrete app/page implementations.
- UI ownership status: page renderers and shared UI primitives have been moved from `src/app/pages` and `src/app/ui` into `src/ui`; `src/app` now composes UI renderers instead of owning their implementation files.
