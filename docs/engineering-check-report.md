...
- UI/input/page-model/lifecycle ownership status: page renderers, shared UI primitives, topbar/list/help chrome, and UI projection structs now live under `src/ui`; page-level input routing lives in `src/app/app_input_router.*`; page titles/rows/list classification live in `src/app/app_page_model.*`; page-level render routing and UI view assembly live in `src/app/app_renderer.*`; app tick ordering lives in `src/app/app_lifecycle.*`; `LoFiBoxApp` delegates input, render, page-model, and lifecycle dispatch through narrow target interfaces.
