...
For product commands and queries that may mutate or read shared app state from GUI, desktop integration, future CLI, automation, or tests, use `application-command-boundary-spec.md`.
...
- expose page-local selected-row, focused-field, or confirm-flow state as a product command API
- store raw key bytes, input-method composition internals, or runtime-shell protocol details as shared app-state truth

Product commands and queries that affect these shared states belong behind the application command/query boundary.
Pages may request those commands through GUI routing, but they do not define the reusable command surface.
