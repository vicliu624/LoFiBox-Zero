...
- `runtime command/session boundary`
  Transport-neutral live runtime command and query bus for playback, active queue, active EQ, active remote session, and live settings. It preserves one live-session truth for GUI, desktop integration, future runtime CLI, automation, MCP-style tools, and tests without treating `AppRuntimeContext`, page rows, or transport as the command model.
- `playback`
...
- GUI, desktop integration, future runtime CLI, automation clients, MCP-style runtime tools, and tests must converge through the runtime command/session boundary defined in `runtime-command-session-architecture-spec.md` whenever they mutate or query live playback, active queue, active EQ, active remote session, or live settings.
...
- Live runtime command/session responsibilities shared by GUI, desktop integration, future runtime CLI, automation clients, MCP-style runtime tools, and tests must follow `runtime-command-session-architecture-spec.md`.
...
- `src/app/app_runtime_context.*` owns GUI runtime composition, runtime services, controllers, the in-process runtime command client composition for the current GUI instance, and the target-interface implementations used by input routing, render routing, lifecycle, and command execution; `LoFiBoxApp` must remain a thin public facade and must not directly include app state, controllers, page model, render router, input router, lifecycle, or command executor headers.
...
- `src/runtime` owns the transport-neutral runtime command/query contract, tagged command payloads, live runtime snapshots, playback/queue/EQ/remote-session/settings runtime domains, snapshot assembly, runtime session facade, in-process runtime command dispatcher, runtime command client/server contract, and transport-neutral envelopes. It may compose application services and current controllers behind runtime domains, but it must not include UI pages, target entry points, direct CLI parsing, concrete platform adapters, or concrete IPC transports.
...
- Future runtime command transport code belongs behind a platform transport boundary and must consume the runtime command/query contract rather than redefining command semantics.
...
- Do not make `targets` or `src/cli` own live runtime command semantics; future live-state terminal control must go through a runtime command client/server over the runtime command/session contract.
