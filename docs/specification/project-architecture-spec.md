...
- `src/runtime` owns the transport-neutral runtime command/query contract, live runtime snapshots, runtime session facade, and in-process runtime command dispatcher. It may compose application services and current controllers behind the facade, but it must not include UI pages, target entry points, direct CLI parsing, concrete platform adapters, or transport implementations.
