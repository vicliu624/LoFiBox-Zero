...
- `src/app/runtime_services.*` owns the runtime capability registry; runtime services must be grouped as connectivity, metadata, playback, remote, cache, and MIDI capabilities instead of exposing top-level provider fields.
...
- `RuntimeServiceRegistry`, `ConnectivityServices`, `MetadataServices`, `PlaybackServices`, `RemoteMediaServices`, `CacheServices`, and `MidiServices` are valid runtime injection boundaries; new providers must join the correct capability group instead of adding flat global fields.
