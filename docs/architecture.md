...
`src/audio/audio_pipeline_controller.*` owns the app-facing audio pipeline facade for backend command dispatch, playback state reads, and visualization tap reads. Playback runtime code calls this facade rather than directly owning backend/visualization adapters.
`src/remote/common/remote_provider_contract.*` owns provider manifests/capabilities; `src/app/remote_profile_store.*` and `src/platform/host/xdg_remote_profile_store.*` own profile persistence with credential references only. `src/app/media_item.*`, `media_search_service.*`, and `mixed_playback_queue.*` define the local/remote media-item bridge. `src/library/library_indexer.*` and `library_store.*` own indexing and durable-store boundaries. `src/metadata/enrichment_authority_policy.*` owns enrichment trust decisions. `src/audio/dsp/dsp_chain.*` owns DSP node state and sample-processing policy.
`src/platform/host/runtime_remote_media_tool.*` owns the app-to-helper boundary for real media-server operations; the helper implements Jellyfin, OpenSubsonic/Navidrome-compatible, and Emby probe, catalog, search, recent/latest, and stream-resolution calls without leaking protocol details into playback or UI.
