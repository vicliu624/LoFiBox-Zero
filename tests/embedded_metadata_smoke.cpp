...
int main()
{
    ...
    const auto mojibake_metadata = lofibox::platform::host::runtime_detail::parseMetadataOutput(
        "TAG:title=è¯´å¥½ä¸å“­ (with äº”æœˆå¤©é˜¿ä¿¡)\n"
        "TAG:artist=å‘¨æ°ä¼¦\n"
        "TAG:album=è¯´å¥½ä¸å“­ï¼ˆwith äº”æœˆå¤©é˜¿ä¿¡ï¼‰\n");
    if (!mojibake_metadata.title || *mojibake_metadata.title != "说好不哭 (with 五月天阿信)") {
        std::cerr << "Expected Latin-1 mojibake title metadata to be repaired.\n";
        fs::remove_all(root, ec);
        return 1;
    }
    if (!mojibake_metadata.artist || *mojibake_metadata.artist != "周杰伦") {
