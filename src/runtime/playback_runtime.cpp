...
PlaybackRuntimeSnapshot PlaybackRuntime::snapshot(std::uint64_t version) const
{
    ...
    result.artist = status.artist;
    result.album = status.album;
    result.album_artist = status.album_artist;
    result.source_label = status.source_label;
    result.source_type = status.source_type;
    result.elapsed_seconds = status.elapsed_seconds;
    result.duration_seconds = status.duration_seconds;
    result.seekable = status.seekable;
    result.live = status.live;
    result.audio_active = session.audio_active;
    result.codec = status.codec;
    result.bitrate_kbps = status.bitrate_kbps;
    result.sample_rate_hz = status.sample_rate_hz;
    result.bit_depth = status.bit_depth;
